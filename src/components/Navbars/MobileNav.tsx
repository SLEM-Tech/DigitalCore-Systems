					...
					>
						<div className='flex flex-col gap-2 py-4 px-4 items-center'>
							<button onClick={closeDrawer} className='self-end'>
								<IoMdClose
									size={24}
									className='hover:scale-125 transition text-primary-500'
								/>
							</button>
											...
											Categories.filter(
												...
												return (
													<span
														...
														key={item?.id}
														className='cursor-pointer hover:text-primary-500 transition'
									...
									<div className='flex flex-col gap-4 mt-3 text-base font-semibold text-black'>
										{headerNavLinks.map((link) => (
											<Link
												key={link.id}
												href={link.href}
												className={`text-base w-fit font-[500] leading-[1.8] transition hover:text-primary-500 relative group ${
													pathname === link.href
														? "text-primary-500"
														: "text-black/80"
												}`}
